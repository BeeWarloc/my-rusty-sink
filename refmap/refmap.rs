...
impl<'a> RefMap<'a> {
    fn insert<T: 'a>(&self, key: &'a T, value: &'a T) {
        unsafe {
            self.ref_map.borrow_mut().insert(
                mem::transmute::<&'a T, usize>(key),
                mem::transmute::<&'a T, usize>(value),
            );
        }
    }
    ...
    fn get<T: 'a>(&self, key: &'a T) -> Option<&'a T> {
        unsafe {
            self.ref_map
                .borrow()
                .get(&mem::transmute::<&'a T, usize>(key))
                .map(|ptr| mem::transmute::<usize, &'a T>(*ptr))
        }
    }
